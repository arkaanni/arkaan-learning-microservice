node('node1') {
    ...
    stage('Test and package') {
        def services = params.servicesToBuild
        def repos = services.length() > 0 ? services.split(',') : sh(script: 'git diff-tree --no-commit-id --name-only HEAD', returnStdout: true).split('\n') as String[]
        startBuild(repos)
    }
}
